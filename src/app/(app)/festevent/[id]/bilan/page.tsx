...
async function fetchBilanData(
  ...
  return { festivalName: fe.festival.name, events };
}

export default async function BilanPage({ params }: PageContext) {
  ...
  const data = await fetchBilanData(id, session.user.id);
  if (!data) {
    redirect("/catalogue");
  }

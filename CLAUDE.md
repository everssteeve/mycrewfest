...
MyCrewFest — personal project, currently at greenfield stage. Update this file as the stack and architecture are decided.

## Tooling

**If the project uses Python:**
- Dependency management: `uv`
- Linting/formatting: `ruff` + `black`, strict type hints throughout
- Run tests: `uv run pytest` (single test: `uv run pytest path/to/test.py::test_name`)

**If the project uses Node/TypeScript:**
- Package manager: `pnpm`
- Linting/formatting: `biome` (preferred) or `eslint`, TypeScript strict mode
- Run tests: `pnpm test` (single test: `pnpm test -- --testPathPattern=<name>`)

## Design system

Le design system "Electric Camp" est documenté dans `design/` :

- `design/tokens.css` — variables CSS à importer dans l'app (couleurs, typo, spacing, radii, glows, motion)
- `design/README.md` — documentation complète des tokens, composants, règles d'usage

**Résumé des choix structurants :**
- Dark mode only (`--bg-darker: #0D0E12`), optimisé OLED
- Fonts : Archivo Black (display, uppercase obligatoire) / Space Grotesk (body) / Space Mono (horaires, comptes à rebours)
- 5 accents avec rôles sémantiques stricts : neon `#00FF66` (CTA), cyan `#00E5FF` (crew), pink `#FF007A` (programme), orange `#FF9900` (warnings), red `#FF3355` (danger)
- Glows néon sur éléments actifs uniquement
- Spacing 4 px grid, radius-md 12px pour les cards/boutons

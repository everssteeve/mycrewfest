/**
 * E2E tests for the programme day-density badge.
 * Shows ◉ N% · Xh when the user has selected events.
 */
...
test.describe("Programme — day density badge", () => {
  ...
  test("density badge shows when selection exists", async ({ page }) => {
    const id = await getFirstFestEventId(page);
    if (!id) { test.skip(); return; }
    await page.goto(`/festevent/${id}/programme`);
    await page.waitForLoadState("networkidle");
    ...
    const badge = page.getByTestId("programme-day-density");
